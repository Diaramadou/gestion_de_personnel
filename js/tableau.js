...
// let boxTab = document.querySelector('.box-container')
...
addContactButton.onclick = function (event) {
  ...
  if (!name || !email || !telephone || !tache) {
    alert('merci de tout remplir')
    return
  }
  const newContact = { name, email, telephone,tache }
  contacts.push(newContact)
  setCount(contacts.length)
  setContacts(contacts)
  ...
  // const cell3Text = document.createTextNode(tache)
